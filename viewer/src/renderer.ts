...
// Player palette — tuned for 4p visibility (yellow/orange of the Wong
// palette were too close; swapped yellow → purple and brightened for dark bg)
const PLAYER_COLORS = ['#5EA5FF', '#FF8A4C', '#5EED9F', '#C084FC'];
const NEUTRAL_COLOR = '#666666';

// Canvas palettes. "dark" matches legacy look (black bg, cool-blue grid).
// "light" inverts: white bg with cool-blue low-alpha strokes. Player colors
// (PLAYER_COLORS above) stay the same — saturated enough for both.
// Trajectory alpha varies per-step; we store r/g/b and build rgba at call site.
...
const CANVAS_DARK: CanvasPalette = {
  bg: '#000000',
...
const CANVAS_LIGHT: CanvasPalette = {
  bg: '#ffffff',
...
function trajectoryColor(palette: CanvasPalette, alpha: number): string {
...
// Engine: max 6.0 u/turn, min 1.0, formula from refs/engine/orbit_wars.py:528.
function fleetSpeed(ships: number, maxSpeed = 6.0): number {
...
// Simple linear-extrapolation intercept check (ignores planet rotation).
function computeInboundFleets(target: Planet, fleets: Fleet[]): InboundFleet[] {
...
// For a given fleet, find its most likely destination planet (linear-
// extrapolation, smallest positive projected distance among planets whose
// perpendicular offset is within their radius + buffer).
function computeFleetTarget(fleet: Fleet, planets: Planet[]): {
  planetId: number; planetOwner: number; eta: number; speed: number; distance: number;
} | null {
  const cos = Math.cos(fleet.angle);
  const sin = Math.sin(fleet.angle);
  const speed = fleetSpeed(fleet.ships);
  let best: { planetId: number; planetOwner: number; eta: number; speed: number; distance: number } | null = null;
...
type SelectionEntry = { kind: "planet"; id: number } | { kind: "fleet"; id: number };

function readSelections(): SelectionEntry[] {
  try {
    ...
    // Legacy single-entry shape. Migrate forward.
...
function writeSelections(list: SelectionEntry[]): void {
  if (list.length === 0) localStorage.removeItem("ow-selection");
  else localStorage.setItem("ow-selection", JSON.stringify(list));
}

function toggleSelection(list: SelectionEntry[], entry: SelectionEntry): SelectionEntry[] {
